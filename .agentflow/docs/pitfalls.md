...
- 对策：v0.4 收窄为 init + check + editors + tools + REPL；明确不生成 state、handoff、skill 包管理等旧功能。
...
- 现象：README 写「模型自维护」，用户以为 `flow check` 通过即代表文档已填完、AI 一定遵守规范。
- 根因：混淆了提示词契约与工具能力。
...
| 绑定某一 agent/编辑器做 hook 强制流程 | 违背轻巧、平台中立的地基定位；维护成本高且仍无法彻底强制模型 |
| 文档中使用 emoji 标记边界 | 风格不统一、部分环境渲染差；改用「只写 / 不写」纯文本 |
